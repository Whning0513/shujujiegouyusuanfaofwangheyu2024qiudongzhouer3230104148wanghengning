...
void remove(const Comparable &x, BinaryNode *&t) {
    ...
    // 查找要删除的节点
    if (x < t->element) {
        remove(x, t->left);  // 在左子树中递归查找
    } else if (x > t->element) {
        remove(x, t->right);  // 在右子树中递归查找
    } else {  // 找到要删除的节点
        if (t->left != nullptr && t->right != nullptr) {  // 如果节点有两个子节点
            // 用右子树的最小节点代替当前节点
            BinaryNode *minNode = detachMin(t->right);
            minNode->left = t->left;  // 接管被删除节点的左子树
            minNode->right = t->right;  // 接管被删除节点的右子树
            delete t;  // 删除当前节点
            t = minNode;  // 替换为最小节点
        } else {  // 节点只有一个或没有子节点
            BinaryNode *oldNode = t;
            t = (t->left != nullptr) ? t->left : t->right;  // 直接提升子节点
